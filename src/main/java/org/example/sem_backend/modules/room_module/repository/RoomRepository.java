...
//    Page<Room> findByTypeAndStatus(@Param("type") String type, @Param("status") String status, Pageable pageable);
    ...
    @Query(value = "SELECT * FROM rooms r WHERE " +
            ...
            "LOWER(r.room_name) LIKE LOWER(CONCAT('%', :keyword, '%')))",
            countQuery = "SELECT COUNT(*) FROM rooms r WHERE " +
                    "(:type IS NULL OR :type = '' OR r.type = :type) OR " +
                    "(:status IS NULL OR :status = '' OR r.status = :status) OR " +
                    "(:keyword IS NULL OR :keyword = '' OR " +
                    "LOWER(r.room_name) LIKE LOWER(CONCAT('%', :keyword, '%')))",
            nativeQuery = true)
    Page<Room> findByTypeStatusAndKeyword(@Param("type") String type,
